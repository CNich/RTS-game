...
void HeapPathFinder::down(int a){
	...
	int b = a + 1;
	if(heapDebug) {
        printf("length: %d, b: %d, a: %d\n", length, b, a);
    }
	if (length >= (b * 2 + 1)){
		if(heapDebug) cPrint("first if");
		auto left = *arr[b * 2 - 1];
		auto right = *arr[b * 2 + 1 - 1];
		//////cout << "a: " << a << "\t" << "b: " << b << "\t" << "arr[" << b - 1 << "]" << arr[b - 1]->F
		//	<< "\t" << "arr[" << b * 2 - 1 << "]" << arr[b * 2 - 1]->F << "\t" << "arr[" << b * 2 + 1 - 1
		//	<< "]" << "\t" << arr[b * 2 + 1 - 1]->F << endl;
		if(heapDebug) cPrint("left: %d, right: %d", left.F, right.F);
		if (left <= right){
			if(heapDebug) cPrint("left <= right");
			if(heapDebug) cPrint("arr[b - 1]: %d, arr[b * 2 - 1]: %d", arr[b - 1]->F, arr[b * 2 - 1]->F);
			if(heapDebug) cPrint("%d", (*arr[b - 1] > *arr[b * 2 - 1]));
			if (*arr[b - 1] > *arr[b * 2 - 1]){
				if(heapDebug) cPrint("left <= right inner");
				swap(b - 1, b * 2 - 1);
				down(b * 2 - 1);
			}
		}
			...
			//		<< "\t" << "arr[" << b * 2 - 1 << "]" << arr[b * 2 - 1]->F << "\t" << "arr[" << b * 2 + 1 - 1
			//		<< "]" << "\t" << arr[b * 2 + 1 - 1]->F << endl;
			if(heapDebug) cPrint("else:");
			if(heapDebug) cPrint("arr[b - 1]: %d, arr[b * 2 + 1 - 1]: %d", arr[b - 1]->F, arr[b * 2 + 1 - 1]->F);
			if(heapDebug) cPrint("%d", (*arr[b - 1] > *arr[b * 2 + 1 - 1]));
			if (*arr[b - 1] > *arr[b * 2 + 1 - 1]){
				if(heapDebug) cPrint("left <= right else inner");
				swap(b - 1, b * 2 + 1 - 1);
				down(b * 2 + 1 - 1);
			}
		}
		...
		if(heapDebug) cPrint("length >= b*2");
		auto left = *arr[b * 2 - 1];
		if(heapDebug) cPrint("%d", (*arr[b - 1] > *arr[b * 2 - 1]));
		if(heapDebug) cPrint("*arr[b - 1]: %d, *arr[b * 2 - 1]: %d", arr[b - 1]->F, arr[b * 2 - 1]->F);
		if (*arr[b - 1] > *arr[b * 2 - 1]){
			if(heapDebug) cPrint("else else inner");
			swap(b - 1, b * 2 - 1);
			down(b * 2 - 1);
		}
	}
